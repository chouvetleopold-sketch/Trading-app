...
st.markdown(
    ...
    <div style="background-color: #6B6B2F; padding: 30px; border-radius: 8px; margin-bottom: 20px; color: white; font-family: sans-serif;">
        ...
        <p style="margin: 0 0 20px 0; font-size: 1.1rem; opacity: 0.9;">Entrez le nom complet d'une entreprise pour analyser ses indicateurs en temps réel.</p>
        <p style="margin: 0 0 5px 0; font-weight: bold; font-size: 1rem;">Entrez le nom de l'entreprise (ex: Total, LVMH, Airbus, Sanofi) :</p>
    </div>
    """, 
    unsafe_allow_html=True
)

# Zone de saisie de texte libre (Par défaut : LVMH)
...
if recherche_utilisateur:
    try:
        # FONCTION INTELLIGENTE : Recherche le code à partir du nom saisi
        recherche = yf.Search(recherche_utilisateur, max_results=5)
        resultats = recherche.quotes
        
        if not resultats:
            st.error(f"❌ Aucune entreprise trouvée pour le nom '{recherche_utilisateur}'. Essayez d'orthographier le nom différemment.")
        else:
            # On récupère le premier résultat pertinent trouvé par Yahoo Finance
            choix = resultats[0]['symbol']
            
            ticker = yf.Ticker(choix)
            info = ticker.info
            
            # Vérification robuste de l'existence des données
            if not info or not any(k in info for k in ['regularMarketPrice', 'currentPrice', 'previousClose', 'longName']):
                st.error(f"❌ Impossible de charger les données financières pour '{recherche_utilisateur}' (Code trouvé : {choix}).")
            else:
                nom_action = info.get('longName', recherche_utilisateur)
                
                # Petit bandeau discret pour indiquer l'action trouvée
                ...
                with tab2:
                    ...
                    col4.caption("*Certaines entreprises européennes affichent ce taux hors taxe.*")
